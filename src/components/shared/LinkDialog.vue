	...
	<Teleport to="body">
		<Transition name="ld-fade">
			<div v-if="modelValue"
				...
				<div class="bg-panel border border-heading/10 rounded-sm shadow-[0_20px_60px_rgba(0,0,0,0.18),0_4px_16px_rgba(0,0,0,0.1)] w-[360px] max-w-[95vw] overflow-hidden"
					...
					<div class="px-[18px] py-[14px] flex flex-col gap-2.5">
						...
						<input
							...
							v-model="url"
							type="url"
							class="w-full px-[10px] py-2 border border-heading/15 rounded-sm text-sm bg-heading/3 text-text outline-none transition-colors duration-150 focus:border-accent focus:ring-2 focus:ring-accent/15"
					...
					<div class="px-[18px] pt-[10px] pb-[14px] flex gap-2 border-t border-heading/7">
						<button type="button"
							class="px-4 py-1.5 rounded-sm text-[0.8rem] font-semibold cursor-pointer border-0 bg-accent text-white transition-colors duration-[120ms] hover:bg-accent/85"
							@click="insert">
							{{ hasLink ? 'Update' : 'Insert' }}
						</button>
						<button v-if="hasLink" type="button"
							class="px-4 py-1.5 rounded-sm text-[0.8rem] font-semibold cursor-pointer border-0 bg-red-100 text-red-600 transition-colors duration-[120ms] hover:bg-red-200"
							@click="remove">
							Remove link
						</button>
						<button type="button"
							class="px-4 py-1.5 rounded-sm text-[0.8rem] font-semibold cursor-pointer border-0 bg-heading/6 text-text transition-colors duration-[120ms] hover:bg-heading/10"
							@click="cancel">
							Cancel
						</button>
					</div>
