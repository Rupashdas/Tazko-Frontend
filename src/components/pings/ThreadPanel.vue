        ...
        <aside v-if="store.activeThread"
            ...
            <div class="flex items-center justify-between px-4 py-3 border-b border-heading/8 shrink-0">
                ...
                <button @click="store.closeThread()"
                        class="w-7 h-7 rounded-sm flex items-center justify-center text-text/50 hover:bg-heading/8 hover:text-heading transition-colors">
                ...
                <div class="flex items-start gap-2.5">
                    ...
                    <div class="min-w-0">
                        ...
                        <img v-if="store.threadParent.type === 'image'" :src="store.threadParent.url"
                             class="mt-1.5 rounded-sm max-h-28 object-cover" />
                    </div>
                </div>
            </div>
            ...
            <div class="px-3 pb-3 pt-2 border-t border-heading/6 shrink-0">
                <div class="flex items-end gap-2 bg-body rounded-sm border border-heading/10 px-3 py-2 focus-within:border-accent/30 transition-colors">
                    <textarea
                        ref="taRef"
                        v-model="reply"
                        @keydown="onKey"
                    ...
                    <button @click="sendReply"
                            :disabled="!reply.trim()"
                            class="w-8 h-8 rounded-sm flex items-center justify-center transition-all shrink-0"
