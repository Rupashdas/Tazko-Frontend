...
<template>
    <div class="shrink-0 px-4 pb-4 pt-2">
        ...
        <Transition name="emoji-pop">
            <div v-if="showEmoji"
                 class="mb-2 bg-panel rounded-sm shadow-xl border border-heading/10 overflow-hidden inline-block">
                <EmojiPickerPanel @pick="pickEmoji" />
            </div>
        </Transition>

        <div class="flex items-end gap-2 bg-panel border border-heading/10 rounded-sm px-3 py-2 shadow-sm focus-within:border-accent/30 focus-within:shadow-accent/5 focus-within:shadow-md transition-all duration-150">
            <!-- Attachment buttons -->
            <div class="flex items-center gap-0.5 pb-1 shrink-0">
                <button @click="clickFile"
                        class="w-8 h-8 rounded-sm flex items-center justify-center text-text/50 hover:text-accent hover:bg-accent/8 transition-colors" title="Attach file">
                    <v-icon name="bi-paperclip" scale="0.9" />
                </button>
                <button @click="showEmoji = !showEmoji"
                        class="w-8 h-8 rounded-sm flex items-center justify-center transition-colors"
            ...
            <button @click="send"
                    :disabled="!text.trim()"
                    class="w-9 h-9 rounded-sm flex items-center justify-center transition-all duration-150 shrink-0 mb-0.5"
                    :class="text.trim()
