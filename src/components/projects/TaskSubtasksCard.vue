...
const addSubtaskItem = () => {
	...
	emit('update:subtasks', [
		...props.subtasks,
		{ id: Date.now(), title: newSubtask.value.trim(), done: false },
	])
	newSubtask.value = ''
...
const toggleDone = (sub) => {
	emit('update:subtasks', props.subtasks.map(s =>
		s.id === sub.id ? { ...s, done: !s.done } : s
	))
}

const removeSubtask = (sub) => {
	emit('update:subtasks', props.subtasks.filter(s => s.id !== sub.id))
}
</script>
	...
	<div class="bg-panel rounded-sm border border-heading/5 p-5">
		...
		<div class="space-y-0.5">
			<div v-for="sub in subtasks" :key="sub.id"
				...
				<button @click="toggleDone(sub)" :class="['w-[18px] h-[18px] rounded-[5px] border-2 flex items-center justify-center shrink-0 transition-all',
		...
		<div v-if="showSubtaskInput" class="flex items-center gap-2 mt-3 pt-3 border-t border-heading/5">
			...
			<button @click="addSubtaskItem" class="tazko-btn">
