...
const props = defineProps({
	...
	canUpdate:         { type: Boolean, default: false }, // owns labels (also used for adding new ones)
...
// Emits minimal patches the parent turns into API calls:
//   save              → partial task patch ({status, priority, due_date, assignee_ids, label_ids})
//   create-label      → {name} → parent calls POST /labels then adds to task
...
const setDueDate = (date) => {
	...
	// AppDatePicker may emit null for "cleared"
...
const toggleAssignee = (member) => {
	...
	const exists = props.task.assignees.some(a => a.id === member.id)
	const nextIds = exists
		? props.task.assignees.filter(a => a.id !== member.id).map(a => a.id)
		: [...props.task.assignees.map(a => a.id), member.id]
...
const addCustomLabel = async () => {
	...
	// Defer to parent: it creates the label, waits for the id, then saves label_ids.
	...
	<aside class="w-64 shrink-0 space-y-3 sticky top-24 self-start">
		...
		<div class="bg-panel rounded-sm border border-heading/5 p-4">
			...
			<div v-if="task.assignees.length" class="space-y-2 mb-3">
				<div v-for="a in task.assignees" :key="a.id ?? a.initials" class="flex items-center gap-2.5">
			...
			<div v-if="canAssign && members.length" class="flex flex-wrap gap-1.5 pt-3 border-t border-heading/5">
				<button
